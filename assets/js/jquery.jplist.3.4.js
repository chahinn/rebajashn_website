	...
	return this.each(function() {            
		...
		function DropDownClass(){
			...
			this.init_dd = function(el){
				...
				var panel, ul;
				//esto lo inhabilite para que no salga en la lista
				el.prepend("<div class='panel' style='visibility:hidden'>" + val + "</div>");
		...
		function PagingClass(){
			...
			this.updateInfo = function(start, end, all){
				p_hndl.pageinfo.html((eval(start) + 1) + " - " + eval(end) + " de " + eval(all));
			};
		}
		//end of Paging class ---------
