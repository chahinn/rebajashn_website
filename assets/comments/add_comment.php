<?php
	include('dbcon.php');
	$comment_text= isset($_GET['comment_text']) ? $_GET['comment_text'] : "";
	...
	if(checkValues($comment_text))
	{
		$userip = $_SERVER['REMOTE_ADDR'];
		
		mysql_query("INSERT INTO el_comments_post (post_id,comments,userip,date_created) VALUES('".$post_id."','".checkValues($comment_text)."','".$userip."','".strtotime(date("Y-m-d H:i:s"))."')");
		
		$result = mysql_query("SELECT *,
		UNIX_TIMESTAMP() - date_created AS CommentTimeSpent FROM el_comments_post order by c_id desc limit 1");
	}
		...
		<div id="record-<?php  echo $rows['c_id'];?>" align="left">
			   <?php 
					      if($rows['img']==""){
							 ...
							 echo "<img src='../../../members/img_users/$rows[img]' width='40' class='CommentImg' style='float:left;' alt='' />";
							};
						?>
