...
$oferta_id = isset($_GET['id']) ? $_GET['id'] : "";

$oferta_vista = isset($_GET['visto']) ? $_GET['visto'] : "";

if($oferta_id=="") header('Location: elevation19_eventos.php');

//buscar la informacion del cupon
 if ($ciudad=="" && $cupon=="")
   {
     $ciudad=$_SESSION['SESS_CIUDAD'];
	 //$sql_cupon="SELECT * FROM el_cupones where ciudad=$ciudad and ofertadeldia='si' and (NOW()>=ofertadiadesde and NOW()<=ofertadiahasta) and esevento='si' limit 1";
	 $sql_cupon="SELECT * FROM el_cupones  where esevento='si' and id=".$oferta_id;
	 //echo $sql_cupon;
	 ...
	 $row = $db->query($sql_cupon); 
		if($db->affected_rows > 0){
		  $oferta = $db->query_first($sql_cupon);
		   ...
		   header('Location: elevation19_eventos.php');
		 };
	 */
 ...
 //Sumar una a la seccion de vistos
$data['visto'] = "INCREMENT(1)";
$db->update('el_cupones', $data, "id='$oferta_id'");     
		...
		<table class="table table-bordered">
			    	<thead>
			    		<tr style="background-color:#f5f5f5;">
			    			<th><span style="margin-right:10px;font-size:14px" class="label label-success"> &uacute;ltimos Eventos</span></th>
			    		</tr>
			    	</thead>
			    	<tbody>			    	
					...
					<?php
					  ...
					  $sql_ultimas = "SELECT * FROM el_cupones where activo='1' and esevento='si'  ORDER BY id DESC  LIMIT 0,3";
					  //echo $sql_ultimas;
					...
					<div class="extraDealMulti">
									   ...
									   <div class="value">
											   <!--
											   <span class="tachado"><?php echo $record_ultimas['anterior'] ?></span></a>
											   -->
											   </div> 
											
										   <div class="extraDealView">
											<a href="elevation19_evento.php?visto=1&id=<?php echo $record_ultimas['id'];?>">
											 <button class="btn btn-mini btn-success">Ver Evento </button>
											 </a>
											 </div>
...
<table class="table table-bordered">
                   			...
                   			<td>
                    			<div class="score_container_right">
							        ...
							        <div id="score_step_info" class="score_info">
										<?php
					  //listado de las ultimas 3 ofertas ingresadas
					  $sql_ultimas = "SELECT * FROM el_cupones where activo='1' and esevento='si' ORDER BY visto DESC  LIMIT 0,3";
					  //echo $sql_ultimas;
  					  $rows_ultimas = $db->query($sql_ultimas);
						while ($record_ultimas = $db->fetch($rows_ultimas)) {
					?>
					...
					<div class="extraDealMulti">
									   <div class="extraDealDescription">
										 <h4><a href="elevation19_evento.php?visto=1&id=<?php echo $record_ultimas['id'];?>"><?php echo $record_ultimas['titulo'] ?></a></h4>
										 <p><?php echo $record_ultimas['brevedescripcion'] ?></p>
									   </div>
									   ...
									   <div class="value">
										   ...
										   <div class="extraDealImage">
												<a title="" class="jSideDeal" href="elevation19_evento.php?visto=1&id=<?php echo $record_ultimas['id'];?>">
												<img width="85" height="55" src="img_cupones/<?php echo $record_ultimas['img'];?>" class="thumbnail">
												</a>
											</div><br/>
									   </div>
										...
										<div class="row-fluid">
										  ...
										  <div class="span4">
											  ...
											  <div id="div3" class="info_box1"> 
												...
												-->
												<h1 style="color:black;font-size: 3em;padding-top:10px">EVENTO </h1>
												</br/>
											  </div>
											  <!--boton-->
											  <div id="div5" class="info_box"> 
											     <?php $oferta_id=$oferta['id'];?>
												<a class="btn btn-primary btn-large btn-success"  style="height: 30px; width: 200px;font-size:22px"  href="elevation19_obtener_oferta.php?ofertaid=<?php echo $oferta_id; ?>"><img src="../assets/img/oferta2.png"></a>
											  ...
											  <span class="countdown_row countdown_show3">
											   ...
											   <a data-toggle="modal" href="#example" style="text-decoration:none;">&nbsp;
												   <h4 style="padding-top:30px;padding-bottom:10px"> <img src="../assets/img/icon_gift_blue.gif" alt="" />Recomi&eacute;ndalo a un amigo. </h4></a>
											   </div>
											   <!--Tiempo de espera-->	
											   ...
											   <div id="div3" class="info_box2" > 
											     <br /> <br /> 
												 <h3 style="padding-top:18px">Planean Asistir <?php $adquiridos=$oferta['adquiridos']+$oferta['impresos']; echo $adquiridos;?></h3>
												
												    
												 <?php
												  if($oferta['limiteadquirir']==0){
												    echo '<h4 style="padding-top:18px;padding-bottom:18px;"><span><img alt="" class="ib" height="28" src="../assets/img/check_mark.png?SxxwvrIa" title="" width="27">El evento esta Activo!</span></h4>';
												  }else{	
													$adquiridos=$oferta['adquiridos']+$oferta['impresos'];
													if($adquiridos>=$oferta['limiteadquirir'])
														{
															echo '<h4 style="padding-top:18px;padding-bottom:18px;"><span><img alt="" class="ib" height="28" src="../assets/img/check_mark.png" title="" width="27">El evento esta Activo!</span><br/><span style="font-size:12px;color:#0099FF">&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;Cantidad disponible limitada</span></h4>';
												  		}else{
															...
															<?php	
																echo '<h4 style="padding-top:18px;padding-bottom:18px;"><span> El evento aun NO esta Activo! <br/> <h6>Falta que '. $faltan.'  personas que confirmen su asistencia para activarlo. </h6></span></h4>';
  ...
  js.src = "//connect.facebook.net/es_LA/all.js#xfbml=1&appId=409195572483086";
  fjs.parentNode.insertBefore(js, fjs);
}(document, 'script', 'facebook-jssdk'));</script>
   <?php $url="http://elevation19.com/members/elevation19_evento.php?visto=1&id=".$oferta_id;?>

<div class="fb-like" data-href="<?php echo urlencode($url); ?>" data-send="true" data-layout="button_count" data-width="450" data-show-faces="true"></div>
															<p> </p>
          ...
          <div class="row-fluid">
			...
			<div class="row-fluid">
				  <div class="span12">
					...
					<div class="row-fluid">
					  <div class="span8"><h2>Comentarios sobre el evento</h2></div>
					  <div class="span4">
					  			<!--votos-->  
												<?php
											     ...
											     ?>	
							 <h6>Calificaci&oacute;n del Evento</h6>
			 ...
			 <form  name="form-comentar" id="form-comentar" onSubmit="return false;" >
			  ...
			  <?php
						...
						echo "<div class='tweet_box' id='co_$row[0]'>";?> <a class='close'onclick='$("#data_<?php echo $row['0']?>").show();'> &nbsp;</a><?php
						...
						<button class="btn btn-info more" href="#" onclick="javascript:carga_comentarios(3,<?php echo $oferta['id']?>)">Mostrar m&aacute;s comentarios </button>
			        </div>
			  	<br clear="all" />
            ...
            <div class="span4">
              <table class="table table-bordered">
				...
				<div align="center">
					<a class="btn-large btn-danger more" href="elevation19_eventos.php" style="font-size:20px"  >Mostrar m&aacute; Eventos </a>
				</div>	
			  </div><!--/span-->
...
<?php include('elevation19_footer.php');?>
	...
	$(document).ready(function(){	
		...
		$("#comentar").click(function(event){
			...
			if(voto=="" || comentario=="")
			  { alert ("Debe ingresar un comentario y su calificaci&oacute;n para el cup&oacute;n")}
			  else
			  {
			   	//alert(str);
